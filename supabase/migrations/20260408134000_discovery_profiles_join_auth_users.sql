-- Only suggest profiles whose id exists in auth.users so profile_interactions.target_id FK succeeds.
...
comment on function public.get_discovery_profiles(int) is
  'Random profiles (must exist in auth.users) excluding self and already-swiped targets.';
