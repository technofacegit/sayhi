-- Align swipe target validation with get_discovery_profiles: require both public.profiles
-- and auth.users (same as inner join used in discovery). Avoids edge cases where auth.users
-- alone might not match how the app picks targets.
...
comment on function public.save_profile_swipe(uuid, text) is
  'Like/dislike; target must exist in profiles and auth.users; preserves is_favorite.';
