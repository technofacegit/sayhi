-- Paginated profiles the current user has liked (profile_interactions.swipe = 'like').

create or replace function public.get_liked_profiles_page(
  p_limit int,
  p_offset int
)
returns jsonb
...
comment on function public.get_liked_profiles_page(int, int) is
  'Paginated profiles the viewer has liked (same member JSON as lobby page).';

grant execute on function public.get_liked_profiles_page(int, int) to authenticated;
