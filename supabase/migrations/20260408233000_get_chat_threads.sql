-- Chats tab: mutual likes (viewer <-> target both liked) as chat threads.
    ...
    (
      ...
      from (
        select
          ...
          p_target.avatar_url,
          greatest(pi_me.updated_at, pi_other.updated_at) as last_message_at
...
comment on function public.get_chat_threads(int) is
  'Mutual-like chat thread list for current user (profiles + last interaction timestamp).';
