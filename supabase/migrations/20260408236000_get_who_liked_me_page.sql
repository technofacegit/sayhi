-- Paginated profiles who liked the current user (incoming likes).

create or replace function public.get_who_liked_me_page(
  p_limit int,
  p_offset int
)
returns jsonb
...
begin
  ...
  inner join public.profiles p on p.id = pi.viewer_id
  inner join auth.users u on u.id = pi.viewer_id
...
comment on function public.get_who_liked_me_page(int, int) is
  'Paginated profiles who liked the current user (incoming likes).';

grant execute on function public.get_who_liked_me_page(int, int) to authenticated;
