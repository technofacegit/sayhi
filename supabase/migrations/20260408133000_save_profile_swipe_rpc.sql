-- Client-side upsert on profile_interactions can fail under RLS/merge edge cases.
-- Single RPC: viewer = auth.uid(), preserves is_favorite when updating swipe.
...
comment on function public.save_profile_swipe(uuid, text) is
  'Like/dislike for current user toward target; keeps existing is_favorite.';
